...
from football_modeling import tools, model, team, fetch_data

def evaluator(data_dir=getcwd(), games_dir=getcwd(), modeling_data_dir=getcwd()):
    evaluator_obj = Evaluator(data_dir=data_dir, games_dir=games_dir, modeling_data_dir=modeling_data_dir)
    return evaluator_obj

class Evaluator:
    def __init__(self, data_dir=getcwd(), games_dir=getcwd(), modeling_data_dir=getcwd()):
        ...
        if data_dir_check or games_dir_check or modeling_data_dir_check:
            raise Warning("data_dir, games_dir, and modeing_data_dir are not unique.")
    ...
    def __get_full_game_list(self, team_names, model_timeline):
        desired_data = ["id", "away_team", "away_points", "home_team", "home_points", "neutral_site", "season"]
        Game = namedtuple('Game', 'id away_team away_points home_team home_points neutral_size season')
        full_game_list = []
        for team_name in team_names:
            csv_filename = team_name + "_games_data_" + str(model_timeline[0]) + "-" + str(model_timeline[1]) + ".csv"
    ...
    def download_game_data(self, model_timeline, print_progress=False):
    ...
    def download_model_data(self, data_timeline, data_type='drives', print_progress=False):
    ...
    def evaluate_model(self, eval_model, model_timeline, data_timeline, data_type='drives', print_progress=False):
        ...
        full_game_list = self.__get_full_game_list(fbs_teams_list, model_timeline)
